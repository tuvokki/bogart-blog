  ...
  for (; i < len; i++) {
    ...
    switch (this.state) {
      ...
      case 'header_name':
        switch (ch) {
          case COLON:
            this.header = buff.toString('utf8', 0, this.pos);
            this.pos = 0;
      ...
      case 'header_val_end':
        switch (ch) {
          case LF:
            val = buff.toString('utf8', 0, this.pos);
...
util.inherits(MultipartLimitError, Error);
function MultipartLimitError(msg) {
  Error.call(this, msg);

  this.name = 'MultipartLimitError';
  this.message = msg || 'Filesize Limit Reached.';
}
